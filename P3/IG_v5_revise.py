...
def continuous_data(data_fil,line_num,weight):
    ...
    if np.size(np.unique(all_label)) == 1:
        ...
        for i in range(rows):
            if (data_arranged[i-1,-2] != data_arranged[i,-2]) and (i != 0 and i !=  [rows]):
                count = 0
                for j in range(i):
                    if all_label[j] == 1:
                        count += weight[j]
                if sum(weight[0:i-1]) != 0 and sum(weight[i:-1]) != 0:
                    values_ratio.append( [data_arranged[i,line_num]/2+data_arranged[i-1,line_num]/2, sum(weight[0:i-1]), count /sum(weight[0:i-1])] )
                    values_ratio.append([data_arranged[i,line_num]/2+data_arranged[i-1,line_num]/2, 1-sum(weight[0:i-1]), (sum(weight_with_poslabel)-count)/sum( weight[i:-1] )  ])
                else:
...
def information_gain(data_fil, hyx, weight):
    ...
    ratio_pos = sum(weight[np.where(label_column == 1)]) / sum(weight)
