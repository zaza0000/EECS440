...
def info_gain_or_gain_ratio(attribute_index, dataset, use_gain_ratio = 0):
    ...
    if (dataset.schema[attribute_index].type == 'CONTINUOUS'):
        data_arranged, value_ratio = continuous_data(data_array, attribute_index, weight)
        threshold, hyx = find_minima_threshold(value_ratio)

    else: 
        data_arranged,value_ratio = discrete_data(data_array, attribute_index, weight)
...
class build_DecisionTree(object):
    ...
    def __init__(self, max_depth=0, eps=0, trainning_dataset=None, use_gain_ratio=0):
...
# ====================================
    def generate_children(self, feature_index, feature_value, dataset, current_depth):
        ...
        if(attri_type == "CONTINUOUS"):
            ...
            next_feature_index = None
            next_feature_value = None
            if(current_depth == self.max_depth):  # stop when there IG = 0 or reach the max depth
            ...
            if(next_feature_index == None):  # stop when there IG = 0 or reach the max depth
                child_0.is_pureNode = True
                new_children[0] = child_0
            else:
    ...
    def buildTree(self, dataset):
        ...
        self.attri_indices = [i for i in range(1, len(dataset.schema.features[1:-1])+1)]
...
def boosting(max_depth, eps, training_dataset, testing_dataset, use_gain_ratio, iter, Weight):
	global weight
	weight = Weight
	alpha_list = []
	label_list = []
	for i in range(iter):
		tree = build_DecisionTree(max_depth, eps, training_dataset, use_gain_ratio)
		e, pre = tree.classify_dataset(training_dataset)
		alpha = 0.5 * np.log((1 - e) / e)
		alpha_list.append(alpha)
		ee, pred_test_label = tree.classify_dataset(testing_dataset)
		label_list.append(pred_test_label)
		j = 0
		weight_sum = 0
