...
def backward_festure_selection(datasets = None, algorithm = 1, testing_set = None):
...
def dtree(training_set, testing_set):
	...
	remain_features = list()
	feature_set = dict()
	is_improved = 1
	max_feature_num = len(training_set[0])-2
	for i in range(max_feature_num):
		feature_set[i+1] = 0
		remain_features.append(i+1)

	pre_error_rate = -1

	time_start = time.time()
	while(len(feature_set) != min(NUM_OF_FEATURES, max_feature_num) and is_improved == 1):
		feature_set = three_fold_val_dTree(feature_set, training_set, remain_features)
		#print(feature_set)
		...
		if(pre_error_rate != -1):
			if(pre_error_rate >= new_error_rate):
				...
				remain_features.remove(attriIndex)
	...
	prediction = list()
	tree = build_tree_BFS.build_DecisionTree(MAX_DEPTH, EPS, training_set, ENABLE_GAIN, attri_indices = remain_features)
	...
	print("remain_features: ", end=' ')
	for key in remain_features:
		print(key, end=' ')
	print()
...
def three_fold_val_dTree(feature_set, training_data, remain_features):
	for key in feature_set:
		temp = copy.deepcopy(remain_features)
		temp.remove(key)
		...
		for i in range(3):
			...
			# max_depth = tree.get_tree_depth()
			# first_feature_index = tree.get_root().get_attriIndex()
			# first_feature = training_data.schema.features[first_feature_index].name
			# #prediction = list(map(tree.classify_data, testing_set))
			# print('Size: %d ,Maximum Depth: %d ,First Feature: %s' % (size, max_depth, first_feature))
...
def three_fold_val_LR(feature_set, training_data, schema):
	for key in feature_set:
		temp = np.delete(training_data, key, axis=1)
...
def updata_dataset(attriIndex, training_data, testing_set):
	training_data = np.delete(training_data, attriIndex, axis=1)
	testing_set = np.delete(testing_set, attriIndex, axis=1)
	feature_set = dict()
	max_feature_num = len(training_data[0])-2
	for i in range(max_feature_num):
		feature_set[i+1] = 0
	return feature_set, training_data, testing_set
...
def logreg(dataset, testing_set):
	...
	time_start = time.time()
	while(len(feature_set) != min(NUM_OF_FEATURES, max_feature_num) and is_improved == 1):
		feature_set = three_fold_val_LR(feature_set, training_data, data.schema)
		attriIndex = min(feature_set,key=feature_set.get) 
		new_error_rate = feature_set.get(attriIndex)
		if(pre_error_rate != -1):
			if(pre_error_rate >= new_error_rate):
				#print("previous error rate: ", pre_error_rate, "after delete a feature(the best): ", new_error_rate)
				pre_error_rate = new_error_rate
				feature_set, training_data, testing_data = updata_dataset(attriIndex, training_data, testing_data)
				del remain_features[attriIndex-1]
			else:
				is_improved = 0
		else:
			pre_error_rate = new_error_rate
	...
	print("Time consumed:", time_, "s")
	prediction = list()
	lg = Logistic_Regression(lambdaa = LAMBDA, training_data = training_data, schema = data.schema, iteration = ITER2, learning_rate = LR)
	pre, true_label = lg.classify_data(testing_data, data.schema)
	prediction = pre
	print("remain_features: ", end=' ')
	for key in remain_features:
		print(key, end=' ')
...
def cv(datasets):
	...
	for i in range(5):
		...
		shuffle(train_data)
		shuffle(val_data)
		probs, true_label = backward_festure_selection(datasets = train_data, algorithm = ALGORITHM, testing_set = val_data)
