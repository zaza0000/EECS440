...
from nbayes import *
from Naive_Bayes_boosting import boosting_updata_weight
import mldata
import numpy as np
...
# ====================================
def normalize_and_encode(original_data, schema):
    data = original_data
...
def boosting_weights(examples):
    weight = 1/ len(examples) * np.ones(len(examples))
...
# ==================================================
def optimize_function(examples, labels, initial_weights, bias, lambdaa, iteration, learning_rate, boost_w):
    new_weights = initial_weights;
    new_bias = bias;
    #print(loss_function(initial_weights, bias, lambdaa, examples, labels))
    while(iteration > 0):
        iteration -= 1
        db = derivative_bias(initial_weights, bias, examples, labels, boost_w)
        new_bias = bias - learning_rate*db
        for i in range(0, len(initial_weights)):
            dw = derivative_weight(initial_weights, i, bias, lambdaa, examples, labels, boost_w)
            new_weights[i] = initial_weights[i] - learning_rate*dw

    #print(loss_function(initial_weights, bias, lambdaa, examples, labels))

    return new_weights, new_bias

def derivative_bias(weights, bias, dataset, label, boost_w):
    db = 0
    for data_index in range(0, len(dataset)):
        sig = sigmoid(np.dot(weights, dataset[data_index ])+bias)
        db += boost_w[data_index]*(sig - label[data_index])

    return db

def derivative_weight(weights, weight_index, bias, lambdaa, dataset, label, boost_w):
    temp = 0
    for data_index in range(0, len(dataset)):
        sig = sigmoid(np.dot(weights, dataset[data_index])+bias)
        temp += boost_w[data_index]*(sig - label[data_index])*dataset[data_index][weight_index]
...
class Logistic_Regression(object):
    ...
    def __init__(self, lambdaa=1, training_data = None, schema = None, iteration = 10, learning_rate = 0.05, boosting = False):
        ...
        self.episilon = 0.49
        self.temp_w = np.ones(len(training_data)).reshape(-1,1)
        self.boost_w = np.ones(len(training_data)).reshape(-1,1)
        if(self.boosting == True):
            self.boost_w = boosting_weights(training_data)
        self.schema = schema

        normalized_data, data_label, stds, means = normalize_and_encode(training_data, self.schema)
        self.training_data = training_data
        self.normalized_training_data = normalized_data
        self.training_data_label = data_label
        self.means = means
        self.stds = stds
    ...
    def train_weights(self, normalized_data, data_label):
        ...
        self.weights, self.bias = optimize_function(normalized_data, data_label, self.weights, self.bias, self.lambdaa, self.iteration, self.learning_rate, self.temp_w)
        #optimize_function(examples, labels, initial_weights)

    def train_weights_oneMore_iter(self):
        self.weights, self.bias = optimize_function(self.normalized_training_data, self.training_data_label, self.weights, self.bias, self.lambdaa, 1, self.learning_rate, self.boost_w)

    def updata_boosting_w(self):
        if(self.boosting != True):
            return None
        if(self.episilon > 0.5 or self.episilon == 0 or self.episilon == 0.5):
            print("episilon =", self.episilon, "  greater than 0.5, or episilon equal to 0")
            return None
        prob_prediction, _ = self.classify_data(self.training_data)
        prediction = np.zeros([len(prob_prediction),1])
        for i in range(len(prob_prediction)):
            if(prob_prediction[i] >= 0.5):
                prediction[i] = 1

        error_ind, a = np.where(prediction- self.training_data_label)
        self.boost_w, episilon, alpha = boosting_updata_weight(self.boost_w, error_ind ,prediction, self.training_data_label)
        self.episilon = episilon[0]
        if(episilon[0] > 0.5 or episilon[0] == 0 or episilon[0] == 0.5):
            return False, alpha
        return True, alpha

    def classify_data(self, examples, schema):
...
def main():
    ...
    if(ENABLE_VAL == 1):
        ...
        lg = Logistic_Regression(lambdaa = LAMBDA, training_data = original_data, iteration = ITER2, learning_rate = LR, boosting = True)
